...
def test_syndrome_with_core(capsys):
    ...
    # -- SINCE behave v1.4.0: "sequence<text>" became "text"
    cmdline_tags = "(fish or fries) and (beer and water)"
...
@pytest.mark.parametrize("tags_option", [
    '--tags="(fish or fries) and (beer and water)"',
])
def test_syndrome_functional(tags_option, capsys):
    """Verifies that the issue is fixed."""
    command_args = [tags_option, "--tags-help", "--verbose"]
