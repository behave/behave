class TagExpression(object):
    ...
    def __repr__(self):
        ...
        #
        #     or_expression = "Or(%s)" % ",".join(or_terms)
        ...
        if len(self.ands) == 0:
            ...
            for or_terms in self.ands:
                ...
                and_parts.append("Or(%s)" % ", ".join(or_parts))
            expression = "And(%s)" % ",".join([and_part for and_part in and_parts])
            if len(self.ands) == 1:
                expression = and_parts[0]

        # expression = "And(%s)" % ",".join([or_term.split()
        #                                      for or_terms in self.ands
        #                                      for or_term in or_terms])
