...
options = [
    ...
    (('--processes',),
     dict(metavar="NUMBER", dest='proc_count',
          help="""Run scenarios in parallel by creating X number of procs to run them from one queue.
		Not all options work properly under parallel mode. See README.md 
		Even the scenarios generated by Outlines will run in parallel.
		environment.py's before_all will run before any scenarios.
		after_all will run after all scenarios are done.
		before/after scenario/steps, will be parallelized with the scenarios, but still run in order.
		Meaning, before_scenario->scen-beforestep-ario->after_scenario will stil be obeyed.
		""")),
